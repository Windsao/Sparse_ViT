...
import torch.optim
#from vision_transformer import count_epoch
...
def main(args):
    ...
    if args.finetune_layer:
        ...
        for name, params in model_without_ddp.named_parameters():
            if 'norm' in name:
                model_params += [{'params': [params], 'lr': 0.001, 'weight_decay': 1e-4}]
                # print('model params:', name)
        # exit()
        #optimizer =  torch.optim.SGD(model_params, lr=args.layer_lr, momentum=0.9, weight_decay=)
        optimizer =  torch.optim.AdamW(model_params, lr=args.layer_lr, weight_decay=0.05)
