...
# from utils import draw_decomponent_attn, draw_qk, drawHeatMap, visDiag, visDiagList, visOutputList
...
def quant_qk_matmul(query_layer, key_layer, quant_matmul=None):
    do_normalize = False
    if do_normalize:
...
class Attention(nn.Module):
    def __init__(self, dim, SMD=False, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., quant=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.quant:
            ...
            attn = (quant_q @ quant_k .transpose(-2, -1)) * self.scale
        else:
            attn = (q @ k.transpose(-2, -1)) * self.scale

        if self.SMD:
            kapa = (q @ k.transpose(-2, -1)) * self.scale
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(dim, SMD=True, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, quant=True)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)
        if self.dist_token is None:
            return self.pre_logits(x[:, 0]), attn_list
        else:
            return x[:, 0], x[:, 1]
    ...
    def forward(self, x):
        ...
        if self.head_dist is not None:
            ...
            x = self.head(x)
        global diag_list
        # visDiagList(diag_list)
        # visOutputList(diag_list)
        # np.save('taylor_output.npy', np.array(diag_list))
        # exit()
        # return x, attn_list
        return x
